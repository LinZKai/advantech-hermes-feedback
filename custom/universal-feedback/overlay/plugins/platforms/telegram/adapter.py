...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Feedback storage is optional: a migration failure must not stop
        # Telegram's ordinary message handling.
        try:
            self._feedback_store = FeedbackStore()
        except Exception:
            logger.exception("Telegram FeedbackStore initialization failed; feedback disabled")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Keep the manual Feedback Spike command out of the generic agent path.
            self._app.add_handler(CommandHandler("feedback_test", self._handle_feedback_test))
            self._app.add_handler(TelegramMessageHandler(
                filters.COMMAND,
    ...
    async def send_feedback(self, chat_id: str, run_id: str, metadata: Optional[Dict[str, Any]] = None) -> SendResult:
        """Legacy standalone /feedback_test prompt (resolved semantics)."""
        return await self._send_feedback_prompt(chat_id, run_id, metadata=metadata, universal=False)

    async def _send_feedback_prompt(self, chat_id: str, run_id: str, *, metadata=None, universal: bool = True) -> SendResult:
        ...
        try:
            ...
            if universal:
                keyboard = InlineKeyboardMarkup([[
                    InlineKeyboardButton("👍 有幫助", callback_data=f"fb:h:{run_id}"),
                    InlineKeyboardButton("👎 需改善", callback_data=f"fb:u:{run_id}"),
                ]])
                text = "這則回答對您有幫助嗎？"
            else:
                keyboard = InlineKeyboardMarkup([[
                    InlineKeyboardButton("✅ 已解決", callback_data=f"fb:y:{run_id}"),
                    InlineKeyboardButton("❌ 尚未解決", callback_data=f"fb:n:{run_id}"),
                ]])
                text = "這次回答有解決您的問題嗎？"
            kwargs: Dict[str, Any] = {"chat_id": normalize_telegram_chat_id(chat_id), "text": text, "reply_markup": keyboard, **self._link_preview_kwargs()}
            thread_id = self._metadata_thread_id(metadata)
            reply_to_id = self._reply_to_message_id_for_send(None, metadata, reply_to_mode=self._reply_to_mode)
            kwargs["reply_to_message_id"] = reply_to_id
            kwargs.update(self._thread_kwargs_for_send(chat_id, thread_id, metadata, reply_to_message_id=reply_to_id, reply_to_mode=self._reply_to_mode))
            msg = await self._send_message_with_thread_fallback(**kwargs)
    ...
    async def send_universal_feedback(self, chat_id: str, *, metadata: Optional[Dict[str, Any]] = None) -> SendResult:
        """Create and send one universal message-level feedback prompt."""
        meta = dict(metadata or {})
        key = meta.get("turn_key") or turn_key(chat_id, meta.get("user_message_id"))
        meta["turn_key"] = key
        run_id = uuid.uuid4().hex
        result = await self._send_feedback_prompt(chat_id, run_id, metadata=meta, universal=True)
        if result.success:
            return result
        return result
        ...
        if data.startswith("fb:"):
            ...
            if query_chat_id is not None:
                ...
                if row is None or str(row["chat_id"]) != str(query_chat_id):
                    await query.answer(text="This feedback prompt is invalid.")
                    return
                original_user_id = row["telegram_user_id"]
    ...
    async def _handle_feedback_test(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> None:
        """Send the manually-triggered Feedback Spike prompt."""
        msg = self._effective_update_message(update)
        chat = getattr(update, "effective_chat", None)
        if not msg or not chat or getattr(chat, "id", None) is None:
            return
        if not self._should_process_message(msg, is_command=True):
            return
        if not self._is_user_authorized_from_message(msg):
        ...
        if getattr(user, "id", None) is not None:
            metadata["telegram_user_id"] = str(user.id)
        thread_id = getattr(msg, "message_thread_id", None)
        if thread_id is not None:
            metadata["thread_id"] = str(thread_id)
        await self.send_feedback(str(chat.id), run_id, metadata=metadata)
